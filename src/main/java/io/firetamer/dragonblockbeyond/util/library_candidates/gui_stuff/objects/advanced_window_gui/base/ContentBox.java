...
public class ContentBox extends ContentBoxBehaviour implements WidgetEventListener {
    protected ItemRenderer itemRenderer;
    protected Font font;

    protected int boxTopLeftX;
    protected int boxTopLeftY;
    protected int boxEndX;
    protected int boxEndY;

    protected boolean isHovered;

    /**
     * The reason for multiple constructors here is because all other Window elements will be built off of this class.
     * It will not provide functionality for any of those elements, that will be done in the respective classes.
     * Unless you are extending this class to make a new element type, only use this first constructor when adding elements to a Window.
     */
    public ContentBox(BasicBoxProperties properties, InteriorAndExteriorChildren children) {
    ...
    public ContentBox(BasicButtonProperties properties, ExteriorChildren children) {
        super(properties, children);
    ...
    private void iterateInteriorElements(Consumer<ContentBox> consumer) {
        for (ContentBox item : interiorChildren) {
    ...
    private void iterateInteriorButtons(Consumer<BasicButton> consumer) {
        int numItems = interiorButtons.size();

        for (int i = 0; i < numItems; i++) {
            BasicButton item = interiorButtons.get(i);
            consumer.accept(item);
        }
    }
    ...
    public void drawInteriorChildren(PoseStack poseStack, int mouseX, int mouseY, int xIn, int yIn, float zIn, int widthIn, int heightIn) {
        iterateInteriorElements(item -> {
            item.drawInteriorContentBoxes(createDrawingContext(poseStack, widthIn, heightIn, xIn, yIn, zIn), mouseX, mouseY);
        });

        iterateInteriorButtons(item -> {
            item.drawInteriorButtons(createDrawingContext(poseStack, widthIn, heightIn, xIn, yIn, zIn), mouseX, mouseY);
        });
    }
    public void drawInteriorContentBoxes(GuiDrawingContext drawContext, int mouseX, int mouseY) {
        ...
        drawInteriorChildren(drawContext.poseStack, mouseX, mouseY, topLeftXPos, topLeftYPos, drawContext.originZ, contentBoxWidth, contentBoxHeight);
    ...
    public void drawInteriorButtons(GuiDrawingContext drawContext, int mouseX, int mouseY) {
    ...
    public BasicButton getHoveredItem() {
        ...
        for (BasicButton item : interiorButtons) {
            if (item.isHovered())
                return item;
        }
